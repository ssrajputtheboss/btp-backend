...
def get_uploads(user):
    ...
    if os.path.isdir(os.path.join(app.config['UPLOAD_FOLDER'],data["public_id"])):
        files = os.listdir(os.path.join(app.config['UPLOAD_FOLDER'],data["public_id"]))
        files = list(filter(lambda x : x.endswith(".mp4"),files ))
...
def upload_video(user):
    ...
    if file.filename == '':
        ...
        if not os.path.isdir(os.path.join(app.config['UPLOAD_FOLDER'],data["public_id"])):
            os.mkdir(os.path.join(app.config['UPLOAD_FOLDER'],data["public_id"]))
        file.save(os.path.join(app.config['UPLOAD_FOLDER'],data["public_id"], filename))
        ...
        if sigma>100:
            sigma = 100
        task = Thread(target=generate_summary, args=(data["public_id"],filename,sigma,),daemon=True)
        task.start()
